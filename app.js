...
function exportCalendarExcel() {
  ...
  const escapeHtml = (value) => String(value).replace(/[&<>"']/g, (ch) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[ch]));
  const maxRows = Math.max(...CALENDAR_DAYS.map((day) => groups[day].length), 1);
  const body = Array.from({ length: maxRows }, (_, rowIndex) => `
    <tr>${CALENDAR_DAYS.map((day) => {
      const ev = groups[day][rowIndex];
      if (!ev) return "<td></td>";
      return `<td><b>${escapeHtml(ev.code || "-")}</b><br>${escapeHtml(displayTime(ev.time))}<br>${escapeHtml(ev.section || "-")} | ${escapeHtml(ev.room || "No room listed")}<br>${escapeHtml(ev.faculty || "-")}</td>`;
    }).join("")}</tr>
  `).join("");
  const html = `<!doctype html><html><head><meta charset="utf-8"></head><body>
    <h1>${escapeHtml(scheduleTitle())}</h1>
    ...
    <p>Generated: ${escapeHtml(new Date().toLocaleString())}</p>
    <table border="1">
      <thead><tr>${CALENDAR_DAYS.map((day) => `<th>${escapeHtml(formatDayLabel(day))}</th>`).join("")}</tr></thead>
      <tbody>${body}</tbody>
    </table>
  </body></html>`;
...
function exportCalendarImage(format) {
  ...
  const startHour = 7;
  const endHour = 22;
  const gridTop = headerHeight + 64;
  const dayHeaderHeight = 48;
  const leftAxis = 92;
  const colWidth = (width - leftAxis - 48) / 7;
  const height = gridTop + dayHeaderHeight + (endHour - startHour) * hourHeight + 48;
  const canvas = document.createElement("canvas");
  canvas.width = width;
  canvas.height = height;
  const ctx = canvas.getContext("2d");
  ...
  ctx.fillText(`COSFinder - Student ID ${state.studentId || "-"}`, 56, 102);
  ctx.font = "600 16px Inter, Arial, sans-serif";
  ctx.fillText(`Calendar format - Generated ${new Date().toLocaleString()}`, 1040, 102);

  ctx.fillStyle = "#efe5f8";
  ctx.fillRect(leftAxis, gridTop, width - leftAxis - 48, dayHeaderHeight);
  ctx.fillStyle = "#3f1d62";
  ctx.font = "800 17px Inter, Arial, sans-serif";
  CALENDAR_DAYS.forEach((day, index) => {
    const x = leftAxis + index * colWidth;
    ctx.fillText(formatDayLabel(day), x + 16, gridTop + 31);
  });

  const gridStartY = gridTop + dayHeaderHeight;
  ctx.strokeStyle = "#d9c8e8";
  ctx.lineWidth = 1;
  for (let hour = startHour; hour <= endHour; hour += 1) {
    ...
    ctx.fillText(hourToLabel(hour), 28, y + 5);
  }
  for (let i = 0; i <= 7; i += 1) {
    const x = leftAxis + i * colWidth;
    ctx.beginPath();
  ...
  events.forEach((ev) => {
    const range = ev.range || parseTimeRange(ev.time);
    const dayIndex = CALENDAR_DAYS.indexOf(ev.dayCode);
    if (!range || dayIndex < 0) return;
    const x = leftAxis + dayIndex * colWidth + 8;
    const y = gridStartY + Math.max(0, range.start - startHour) * hourHeight + 6;
    const h = Math.max(54, (range.end - range.start) * hourHeight - 12);
    roundRectCanvas(ctx, x, y, colWidth - 16, h, 10, departmentColor(ev.department));
    ctx.fillStyle = "#ffffff";
    ctx.font = "800 15px Inter, Arial, sans-serif";
    ctx.fillText(String(ev.code || "-").slice(0, 13), x + 12, y + 24);
    ctx.font = "700 12px Inter, Arial, sans-serif";
    ctx.fillText(displayTime(ev.time), x + 12, y + 44);
    ctx.font = "600 11px Inter, Arial, sans-serif";
    ctx.fillText(`${ev.section || "-"} | ${ev.room || "No room listed"}`.slice(0, 22), x + 12, y + 62);
  });
...
function exportPdf(layout = "list") {
  const rows = exportRows();
  const lines = layout === "calendar" ? calendarPdfLines() : [
    scheduleTitle(),
  ...
  showToast(`${layout === "calendar" ? "Calendar-format " : ""}PDF schedule saved.`);
}
